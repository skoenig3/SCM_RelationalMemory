function getViewingBehaviorSCMRM(FIXATIONFILE,SAMPRATE,imageX,imageY,PLOTOPTIONS)
% Seth Konig 1/25/16 modified from getViewingBehaviorCF to import data from
% SCMRM
...
if nargin < 2
    SAMPRATE = 1; %in ms/sampled point
end
if nargin < 3
    imageX = 800;
    imageY = 600;
...
transitionthreshold = 9; %was 45 but sampling 5 times higher so divided by 5 also a little smoothing%counting as a turn/rotation
...
for cndlop = 1:2:length(fixationstats); %only uses novel viewing since images changes could alter natural behavior
    ...
    if ~isempty(fixations)
        ...
        nov_allval = per(cndlop).allval;
        nov_alltim = per(cndlop).alltim;
        ...
        if nov_img_off > 20000
           nov_img_off = length(fixationstats{cndlop}.XY);
        end
        xy = fixationstats{cndlop}.XY(:,nov_img_on:nov_img_off);

        
        nov_fix = fixationstats{cndlop}.fixations;
        nov_fixtimes = fixationstats{cndlop}.fixationtimes;
        nov_sactimes = fixationstats{cndlop}.saccadetimes;
        ...
        fixations = nov_fix;
        fixationtimes = nov_fixtimes-nov_img_on;
        saccadetimes = nov_sactimes-nov_img_on;
