function varargout = human_scm12(varargin)
% HUMAN_SCM12 MATLAB code for human_scm12.fig
%      HUMAN_SCM12, by itself, creates a new HUMAN_SCM12 or raises the existing
%      singleton*.
%
%      H = HUMAN_SCM12 returns the handle to a new HUMAN_SCM12 or the handle to
%      the existing singleton*.
%
%      HUMAN_SCM12('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HUMAN_SCM12.M with the given input arguments.
%
%      HUMAN_SCM12('Property','Value',...) creates a new HUMAN_SCM12 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before human_scm12_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to human_scm12_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help human_scm12

% Last Modified by GUIDE v2.5 03-Mar-2015 16:03:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @human_scm12_OpeningFcn, ...
    'gui_OutputFcn',  @human_scm12_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before human_scm12 is made visible.
function human_scm12_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to human_scm12 (see VARARGIN)

% Choose default command line output for human_scm12
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes human_scm12 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = human_scm12_OutputFcn(hObject, eventdata, handles)
...
UserData.response = []; %where we're going to store the data, row1 type, row 2 guess type
image_names = [];
for block = 1:3
    for block_img = 1:12
        img = 12*(block-1)+block_img;
        UserData.response = [UserData.response 1];
        image_names = [image_names {strtrim(image_list(order(1,img),:))}];
    end
    for block_img = 1:12;
        UserData.response = [UserData.response NaN];
        img = 12*(block-1)+block_img;
...
if last_image == 72
    %save the data
    save_dir = 'R:\Buffalo Lab\eblab\Cortex Programs\Scene Manipulation\SCM Picture Sets - SR\List12_responses\';
    pub1_h = findobj('Tag','pushbutton1');%get the hanlde for pushbutton3 that has ROI data
    UserData  = get(pub1_h,'UserData');
    ...
    save([save_dir 'List12_Set' num2str(Setnum) '_' initial],'Responses','image_names','Setnum')
    close all
else
    display_next_image(folder_name,image_names,last_image,handles)
end
...
axis off
pause(1) %inter-trial_interval in seconds
