...
folder_name = uigetdir('C:\Users\seth.koenig\Documents\MATLAB\SCM_RelationalMemory\Image Sets\',...
    'Select Image folder');
...
for il = 1:size(image_list,1);
    num  = sscanf(image_list(il,5:end), '%d');
    %     if num < 10
...
if strcmp(image_list(1,11),' ') %forgot to add 0 for image nums < 10
    for img = 1:72
        ...
        imgnam =image_list(img,5:period-1);
